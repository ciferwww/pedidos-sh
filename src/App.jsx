...
const WHATSAPP_NUMBER = "526441234567"; // ← pon el número real de Shekinah
    ...
    <div style={{
      ...
      {open && (
        <div style={{padding:"0 14px 14px", borderTop:`1px solid ${G.divider}`}}>
          ...
          {item.isSushi && (
            <div style={{marginTop:12}}>
              <Chip label={`💣 Convertir en Bomba +$${BOMBA_PRICE}`} active={bomba}
                onClick={()=>setBomba(!bomba)} danger />
...
function OrderModal({ items, onClose, onSend }) {
  ...
  const doSend = () => onSend({ name, phone, delivery, address, payment }, setSending);
...
export default function App() {
  ...
  // Guarda en Firebase Y abre WhatsApp
  const handleSend = async ({ name, phone, delivery, address, payment }, setSending) => {
    setSending(true);
    const total = cart.reduce((s,i)=>s+i.totalPrice,0);
    try {
      await addDoc(collection(db, "pedidos"), {
        ...
        total,
        estado:    "nuevo",       // nuevo | en_proceso | listo | entregado
        creadoEn:  serverTimestamp(),
      });
    } catch(e) {
      console.error("Error guardando pedido:", e);
    }
    // Siempre abre WhatsApp aunque Firebase falle
    const data = { name, phone, delivery, address, payment };
  ...
  return (
    <div style={{background:G.offWhite, minHeight:"100vh",
        ...
        {cat==="Sushi" && (
          <div style={{background:"#1C1208", borderRadius:8, padding:"9px 13px", marginBottom:14}}>
            <p style={{color:"#eee", fontSize:12, margin:0}}>
              ...
              {" "}Ingrediente extra: <strong style={{color:G.goldLight}}>+$20</strong> rollos · <strong style={{color:G.goldLight}}>+$30</strong> platillos.
            </p>
          </div>
        )}
        {MENU[cat].map(item=>(
      ...
      {showModal && (
        <OrderModal items={cart} onClose={()=>setShowModal(false)} onSend={handleSend} />
      )}
    </div>
  );
}