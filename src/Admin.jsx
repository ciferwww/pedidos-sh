...
// NOTA: el catálogo MENU_POS hardcodeado fue retirado. El POS ahora lee
// en vivo de /tenants/{tenantId}/productos (ver useEffect dentro de POS),
// la misma colección que usa el menú del cliente y que administra
// GestorMenu.jsx. Si necesitas re-importar el catálogo original como
...
// ── POS ──────────────────────────────────────────────────────────────
// El catálogo se lee en vivo de /tenants/{tenantId}/productos (la misma
// colección que administra GestorMenu y que consume el menú del cliente
// en App.jsx). Cualquier cambio ahí se refleja aquí al instante.
  ...
  useEffect(() => {
    const unsub = onSnapshot(
      ...
      (snap) => {
        ...
        snap.docs.forEach(d => {
          const data = d.data();
          if (data.disponible === false) return; // oculto del menú = oculto del POS también
          const c = data.categoria || "Otros";
          ...
          grouped[c].push({
            ...
            sauce: data.sauceOptions || null, // "roll" | "boneless" | null
            isSushi: !!data.isSushi,
            fixedIngredients: !data.hasProtein && !data.hasBurgerProtein && !!data.isSushi,
            hasExtras: !!data.hasExtras,
          });
        });
        Object.keys(grouped).forEach(c => grouped[c].sort((a,b)=>(a.orden??999)-(b.orden??999)));
        setMenu(grouped);
  ...
  return (
    <div style={{display:"flex", flexDirection: isMobile?"column":"row", gap:0, height: isMobile?"auto":"calc(100vh - 60px)", fontFamily:"'Segoe UI',sans-serif"}}>
      <style>{`
        ...
        .pos-item-btn:hover .pos-item-img { transform:scale(1.1); }
              ...
              <button key={item.id} id={`pos-item-${item.id}`} className="pos-item-btn" onClick={()=>addItem(item)} style={{
                ...
                {item.imagen && (
                  <div style={{width:"100%",height:80,overflow:"hidden"}}>
                    <img src={item.imagen} alt={item.name} className="pos-item-img" style={{
