...
import {
  ...
  orderBy, query, addDoc, serverTimestamp, getDoc, setDoc,
  where, getDocs, Timestamp
} from "firebase/firestore";

// ── MISMA config de Firebase ─────────────────────────────────────────
...
function LoginScreen({ onLogin }) {
  ...
  const tryLogin=()=>{ if(pw===ADMIN_PASSWORD)onLogin(); else{setErr(true);setPw("");} };
  return (
    <div style={{minHeight:"100vh",background:G.dark,display:"flex",alignItems:"center",justifyContent:"center"}}>
      <div style={{background:G.offWhite,borderRadius:16,padding:"40px 32px",
        ...
        <button onClick={tryLogin} style={{width:"100%",padding:"12px",borderRadius:9,
...
function OrderCard({ pedido, onChangeEstado }) {
  ...
  return (
    <div style={{background:G.cardBg,borderRadius:12,marginBottom:10,
      border:`2px solid ${est.color}33`,boxShadow:"0 2px 8px #0001",overflow:"hidden"}}>
      <div style={{display:"flex",alignItems:"center",padding:"12px 14px",gap:8,
        ...
        <div style={{flex:1,minWidth:0}}>
          ...
          <p style={{color:G.textSub,fontSize:11,margin:"1px 0 0"}}>{origenLabel}</p>
        </div>
        <div style={{textAlign:"right"}}>
          <p style={{color:G.gold,fontWeight:900,fontSize:16,margin:0,fontFamily:"Georgia,serif"}}>${pedido.total}</p>
...
function POS({ onPedidoCreado }) {
  ...
  const cobrar=async()=>{
    ...
    try{
      ...
      setCart([]); setMesa(""); onPedidoCreado&&onPedidoCreado();
    ...
    <div style={{display:"flex", flexDirection: isMobile?"column":"row",gap:0, height: isMobile?"auto":"calc(100vh - 60px)"}}>
        ...
        <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.6)",zIndex:500,
          ...
          <div style={{background:G.offWhite,borderRadius:14,padding:24,width:320,
              ...
              <div style={{marginBottom:12}}>
                ...
                <div style={{display:"flex",flexWrap:"wrap",gap:5}}>
                  {(showQuick.burgerProtein?PROTEINS_BURGER:PROTEINS_SUSHI).map(p=>(
                    <button key={p} onClick={()=>setTempProtein(p)} style={{
              ...
              <div style={{marginBottom:14}}>
                ...
                <div style={{display:"flex",gap:5,flexWrap:"wrap"}}>
                  {(showQuick.sauce === "roll" ? SAUCES_ROLL : SAUCES_BONELESS).map(s=>(
                    <button key={s} onClick={()=>setTempSauce(s)} style={{
                      padding:"4px 12px",borderRadius:20,cursor:"pointer",fontSize:12,fontWeight:700,
            ...
            <div style={{display:"flex",gap:8}}>
              ...
              <button onClick={confirmQuick} style={{flex:2,padding:"9px",borderRadius:8,
      ...
      <div style={{width: isMobile?"100%":280,maxHeight: isMobile?350:"none",background:"#fff",borderLeft:`2px solid ${G.divider}`,
        ...
        <div style={{padding:"12px 14px",borderBottom:`1px solid ${G.divider}`,background:G.offWhite}}>
          ...
          <input value={mesa} onChange={e=>setMesa(e.target.value)} placeholder="Ej. 3 (dejar vacío = mostrador)"
        ...
        <div style={{padding:"12px 14px",borderTop:`2px solid ${G.divider}`,background:G.offWhite}}>
          ...
          <div style={{display:"flex",justifyContent:"space-between",marginBottom:10}}>
            ...
            <p style={{color:G.gold,fontWeight:900,fontSize:20,margin:0,fontFamily:"Georgia,serif"}}>${total}</p>
          </div>
          <button onClick={cobrar} disabled={sending||cart.length===0} style={{
...
function Cierre() {
  ...
  pedidos.forEach(p=>{ if(porPago[p.pago]!=null) porPago[p.pago]+=(p.total||0); });
  ...
  const exportExcel=()=>{
    const rows=[
      ...
      ["Transferencia:","$"+porPago.transferencia],
      ["Terminal:","$"+porPago.terminal],
      [],
      ["#","Hora","Cliente","Entrega","Pago","Artículos","Total","Estado"],
      ...pedidos.map((p,i)=>[
        i+1,
        ...
        p.pago,
        p.articulos?.map(a=>`${a.cantidad}x ${a.nombre}`).join(" | ")||"",
        "$"+p.total,
  ...
  return (
    <div style={{padding:20,maxWidth:800}}>
      ...
      <div style={{display:"flex",gap:10,marginBottom:16,alignItems:"flex-end"}}>
        ...
        <button onClick={buscar} style={{padding:"9px 20px",borderRadius:8,border:"none",
          background:G.gold,color:G.dark,fontWeight:800,fontSize:14,cursor:"pointer"}}>
          Buscar</button>
        {pedidos.length>0&&(
          <button onClick={exportExcel} style={{padding:"9px 20px",borderRadius:8,border:"none",
      ...
      {!loading&&pedidos.length>0&&(
        <>
          ...
          <div style={{display:"flex",gap:10,marginBottom:16,flexWrap:"wrap"}}>
            {[
              {label:"Total del día",value:"$"+total,color:G.gold},
              {label:"Pedidos",value:pedidos.length,color:G.dark},
              {label:"💵 Efectivo",value:"$"+porPago.efectivo,color:"#27ae60"},
              {label:"📲 Transferencia",value:"$"+porPago.transferencia,color:"#2980b9"},
              {label:"💳 Terminal",value:"$"+porPago.terminal,color:"#8e44ad"},
            ].map(s=>(
            ...
            <table style={{width:"100%",borderCollapse:"collapse",fontSize:13}}>
              <thead>
                <tr style={{background:G.warmGray}}>
                  {["Hora","Cliente","Entrega","Pago","Total","Estado"].map(h=>(
...
function Config() {
  ...
  const save=async()=>{
    await setDoc(doc(db,"config","general"),{deliveryCost:Number(deliveryCost)},{merge:true});
  ...
  return (
    <div style={{padding:20,maxWidth:400}}>
      ...
      <div style={{background:G.cardBg,borderRadius:12,padding:"20px",border:`1px solid ${G.divider}`}}>
        ...
        <button onClick={save} style={{width:"100%",marginTop:12,padding:"11px",borderRadius:9,
...
function Dashboard({ onLogout }) {
  ...
  const pendiente=pedidos.filter(p=>p.estado!=="entregado").reduce((s,p)=>s+(p.total||0),0);
  ...
  return (
    <div style={{minHeight:"100vh",background:"#f0ece4",fontFamily:"'Segoe UI',sans-serif"}}>
      ...
      <div style={{background:G.dark,padding:"10px 20px",borderBottom:`3px solid ${G.gold}`,
        display:"flex",alignItems:"center",height:60,boxSizing:"border-box"}}>
        ...
        <div style={{marginLeft:"auto",display:"flex",alignItems:"center",gap:12}}>
          ...
          <button onClick={onLogout} style={{background:"none",border:`1px solid #444`,
